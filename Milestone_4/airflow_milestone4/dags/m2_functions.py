...
def call_all_dataset_related_functions(df):
...
def extract_additional_reso(df):
    generate_GPS(df)
    map_location_to_gps(
        df,
        [
...
def add_gps(df):
    df["pu_latitude"] = None
    df["pu_longitude"] = None
    df["do_latitude"] = None
    df["do_longitude"] = None
    pickup_locations = df["pu_location"].unique()
    dropoff_locations = df["do_location"].unique()
    cache_df = load_or_create_cache()
    for location in pickup_locations:
        latitude, longitude = get_coordinates(location, cache_df)
        df.loc[df["pu_location"] == location, "pu_latitude"] = latitude
        df.loc[df["pu_location"] == location, "pu_longitude"] = longitude
    for location in dropoff_locations:
        latitude, longitude = get_coordinates(location, cache_df)
        df.loc[df["do_location"] == location, "do_latitude"] = latitude
        df.loc[df["do_location"] == location, "do_longitude"] = longitude
    return df


def load_or_create_cache():
    if os.path.isfile("location_cache.csv"):
        return pd.read_csv("location_cache.csv")
    else:
        return pd.DataFrame(columns=["location", "latitude", "longitude"])


def get_coordinates(location, cache_df):
    if not cache_df.empty:
        cached_data = cache_df[cache_df["location"] == location]
        if not cached_data.empty:
            latitude = cached_data.iloc[0]["latitude"]
            longitude = cached_data.iloc[0]["longitude"]
            return latitude, longitude

    # If not found in cache, make an API request
    # Create a GoogleMaps client.
    googlemaps_client = googlemaps.Client(key="AIzaSyBXV_Q4_CWvV7btH9drTwc3BYRoj2GwozQ")
    try:
        coordinates = googlemaps_client.geocode(location)
        if coordinates:
            ...
            cache_df = pd.concat([cache_df, cache_data], ignore_index=True)
            cache_df.to_csv("location_cache.csv", index=False)
            return latitude, longitude
        ...
        return None, None  # Location not found or error
